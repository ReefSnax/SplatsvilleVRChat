Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/a533e3f61d08f66419ceaff16cffcf70"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.7921569,0.5385657,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.7921569,0.5385657,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.7921569,0.5385657,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0,0,0,0), float(0), float4(1,1,0,0), float(0), float(1));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, float(0), float(0), float4(0,0,0,0), float(1));
