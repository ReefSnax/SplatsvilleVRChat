Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/74343bcf64eec094fab8927a73d2ae32"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0));
				}
				#else
			...
			{
				float smoothness = float(0.808);
				float smoothness2 = float(1);
				float metallic = float(0.595);
			...
			{
				float clearCoatMask = float(0.213);
				float smoothness = float(0.862);
				float reflectionMask = 1;
				float specularMask = 1;
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
				...
				float surfaceReduction = (1.0 / (brdfRoughness * brdfRoughness + 1.0));
				float grazingTerm = saturate(smoothness + (1 - omr));
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, float(0), float(0), _ClearCoatFallback, poiCam.vertexReflectionDir);
			...
			void applyIridescence(inout PoiFragData poiFragData, PoiMesh poiMesh, PoiCam poiCam, inout PoiMods poiMods)
			{
				...
				if (float(0) == 1)
				{
					float hue = float(0) + frac(_Time.x * float(0));
					iridescenceColor.rgb = hueShift(iridescenceColor.rgb, hue);
				}
				#endif
				float iridescenceMask = 1;
				#if defined(PROP_IRIDESCENCEMASK) || !defined(OPTIMIZER_ENABLED)
				iridescenceMask = POI2D_SAMPLER_PAN(_IridescenceMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				#endif
				#ifdef POI_BLACKLIGHT
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, saturate(iridescenceColor.rgb * float(1)), iridescenceColor.a * float(0) * iridescenceMask);
				poiFragData.baseColor.rgb += saturate(iridescenceColor.rgb * float(1) * iridescenceColor.a * float(0) * iridescenceMask);
				poiFragData.baseColor.rgb *= saturate(lerp(1, iridescenceColor.rgb * float(1), iridescenceColor.a * float(0) * iridescenceMask));
				...
				#endif
				poiFragData.emission += saturate(iridescenceColor.rgb * float(1)) * iridescenceColor.a * iridescenceMask * emissionStrength;
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.2411783,0.2411783,0.2411783,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.2411783,0.2411783,0.2411783,1).a;
