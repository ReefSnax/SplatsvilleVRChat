Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/aa3b568ad83a9dd42a8328f661aee6d6"
{
	Properties
	{
 ...
 #define PROP_FLIPBOOKTEXARRAY 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(100,200,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * float(1) * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * float(0.41));
			}
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					if (float(24) != 0)
					{
						currentFrame = (_Time.y / (1 / float(24))) % totalFrames;
						if (float(0))
						{
				...
				poiFragData.baseColor = poiFragData.baseColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * flipBookMask * float(1) * flipbookAlpha);
