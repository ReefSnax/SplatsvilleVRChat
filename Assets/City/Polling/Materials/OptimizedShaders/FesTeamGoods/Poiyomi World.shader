Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/221fe9c7e63f9bc45912d3abd1ff3241"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(0) == 1)
				{
					...
					#endif
					if (float(0.09) == float(0)) float(0.09) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.09), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.09), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.09), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(0) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(0) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.09), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
