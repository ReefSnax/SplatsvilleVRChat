Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/302f8dd2861c750479965777f68f40e9"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(0.544) + float(5) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(5,1.25,0,0), float(0), float4(0.5,1.25,0,0), float(0), float(1));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, float(1), float(0), float4(0,0,0,0), float(1));
