Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/6700e170cb5b8144ab237690d1ece319"
{
	Properties
	{
 ...
 #define PROP_DEPTHTEXTURE 
			...
			void applyVertexColor(inout PoiFragData poiFragData, PoiMesh poiMesh)
			{
				if (float(1))
				{
					...
					poiFragData.baseColor *= lerp(1, vertCol, float(1));
					#endif
					poiFragData.alpha *= lerp(1, poiMesh.vertexColor.a, float(1));
				}
			}
			...
			void MochieBRDF(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.1,0,0.5)), float4(0,0,0,0).xy);
			...
			void poiClearCoat(inout PoiFragData poiFragData, in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float specularMask = 1;
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_ClearCoatMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.1,0,0.5)), float4(0,0,0,0));
				clearCoatMask *= PRBMaps.r;
				smoothness *= PRBMaps.g;
				reflectionMask *= PRBMaps.b;
				specularMask *= PRBMaps.a;
				#endif
				specularMask *= float(0.02);
				reflectionMask *= float(1);
				if (float(0))
				{
			...
			void applyDepthFX(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_DEPTHMASK) || !defined(OPTIMIZER_ENABLED)
				float depthMask = POI2D_SAMPLER_PAN(_DepthMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.1,0,0)), float4(0,0,0,0)).r;
				#else
				float depthMask = 1;
				#endif
				if (float(1))
				{
					float colorBlendAlpha = lerp(float(0), float(1), remapClamped(float(0.75), float(1.34), diff));
					#if defined(PROP_DEPTHTEXTURE) || !defined(OPTIMIZER_ENABLED)
					float2 depthTextureUV = float2(0, 0);
					if (float(0) == 8)
					{
						depthTextureUV = lerp(0, 1, remapClamped(float(0.75), float(1.34), diff));
					}
					else
					{
						depthTextureUV = poiMesh.uv[float(0)];
					}
					float3 depthColor = POI2D_SAMPLER_PAN(_DepthTexture, _MainTex, poiUV(depthTextureUV, float4(1,0.1,0,0.4)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
					#else
					...
					poiFragData.emission += depthColor * colorBlendAlpha * float(1) * depthMask;
				}
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(1,0.1,0,0.5)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.1,0,0.5)), float4(0,0,0,0)), float(1));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1653383,0.005869046,0.01265507,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.1653383,0.005869046,0.01265507,1).a;
				#if defined(PROP_CLIPPINGMASK) || !defined(OPTIMIZER_ENABLED)
				float alphaMask = POI2D_SAMPLER_PAN(_ClippingMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.1,0,0)), float4(0,0,0,0)).r;
				if (float(0))
				{
