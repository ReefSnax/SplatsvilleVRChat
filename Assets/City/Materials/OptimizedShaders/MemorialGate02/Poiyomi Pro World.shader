Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/36a8a9a69e8782c4d85b1d8ddf56b2a4"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0,0,1).rgb, float(0));
				#endif
				if (float(1))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos, float4(0,20,0,0).xyz) * float(20)) + _Time.x * float(10)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(0,20,0,0).xyz, float(10), float(20), float(0), float(0), pos);
					}
				}
				if (float(0))
				{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(1) + float(5) * _Time.x) * float(1));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
