Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/c669c23981a96744080a8a73404ed463"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.6231937,0.5906674,0.5906674,1), float4(0.6231937,0.5906674,0.5906674,1), float(1)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(0.08);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.07165321,0.4150943,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.07165321,0.4150943,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.07165321,0.4150943,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
