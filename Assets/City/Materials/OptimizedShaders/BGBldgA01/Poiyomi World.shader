Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/d5ec5830655943149855583adc8a075c"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.017));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.017), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.017));
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.017));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.017));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.017))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.017))));
				}
