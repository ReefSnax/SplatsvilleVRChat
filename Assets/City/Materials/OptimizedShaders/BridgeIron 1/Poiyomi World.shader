Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/5a3503dcbd0b9634e9304e42ecbfe5be"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				//ifex float(1)==0
				#ifdef GEOM_TYPE_BRANCH
				decalScale = float4(0.015,0.015,0,0);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
				...
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.1,0.05,0,0), float(28) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
				...
				//ifex float(1)==0
				#ifdef GEOM_TYPE_BRANCH_DETAIL
				decalScale = float4(0.025,0.05,0,0);
				decalRotation = 0;
				sideMod = 0;
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				...
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.06,0.07,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				ddxuv = ddx(uv);
				ddyuv = ddy(uv);
				if (any(fwidth(uv) > .5))
				{
