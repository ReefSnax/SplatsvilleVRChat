Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/fac78f349260f8e4b958fa1b7de3c56c"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(4.15);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(60,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					mainUV = sharpSample(float4(0.0078125,0.0625,128,16), mainUV);
				}
