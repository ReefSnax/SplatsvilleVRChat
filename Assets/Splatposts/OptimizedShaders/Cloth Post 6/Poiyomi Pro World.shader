Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/621c49b6ced9d154ea91e126ffb92c2e"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.68,0,0.14)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4433962,0.0361883,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,0.68,0,0.14).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4433962,0.0361883,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0,0.4433962,0.0361883,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,0.66,0,0.17)) + _Time.x * float4(0,0,0,0)).r;
				#else
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(0.5,1.5,0,0), float(0), float4(0.5,1.5,0,0), float(0), float(1));
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, float(1), float(2), float4(0,0,0,0), float(1));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					mainUV = sharpSample(float4(0.0009765625,0.015625,1024,64), mainUV);
				}
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(0.2,1,0,1)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.2,1,0,0)), float4(0,0,0,0)), float(1));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0,0.7615293,0.01751345,0.1490196).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0,0.7615293,0.01751345,0.1490196).a;
				#if defined(PROP_CLIPPINGMASK) || !defined(OPTIMIZER_ENABLED)
				float alphaMask = POI2D_SAMPLER_PAN(_ClippingMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(0.2,1,0,1)), float4(0,0,0,0)).r;
				if (float(0))
				{
