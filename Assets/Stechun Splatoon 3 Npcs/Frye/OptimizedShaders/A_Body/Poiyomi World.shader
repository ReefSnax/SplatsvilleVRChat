Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/4386db9ca16559743bf03ab8e8647e6f"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				decalColor = tex2D(_DecalTexture, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,0.773346,0.04718075,1).rgb, float(0)), float4(1,0.773346,0.04718075,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,0.773346,0.04718075,1).rgb, float(0)), float4(1,0.773346,0.04718075,1).a);
				#endif
				...
				decalColor = tex2D(_DecalTexture1, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(0.09989873,0,1,1).rgb, float(0)), float4(0.09989873,0,1,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(0.09989873,0,1,1).rgb, float(0)), float4(0.09989873,0,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0) + poiLight.nDotV * float(0), float(0));
			...
			void applySubsurfaceScattering(in PoiCam poiCam, inout PoiLight poiLight, in PoiMesh poiMesh)
			{
				...
				float SSS = 1;
				#endif
				float3 vLTLight = poiLight.direction + poiMesh.normals[0] * float(1);
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), float(5)) * float(0.25);
				#ifdef UNITY_PASS_FORWARDBASE
