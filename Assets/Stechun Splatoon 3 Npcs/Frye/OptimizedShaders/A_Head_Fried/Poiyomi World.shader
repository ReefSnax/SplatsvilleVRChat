Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/818328af60b8ef54cb24bdef6615d4a3"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				decalColor = tex2D(_DecalTexture, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,0.7844843,0,1).rgb, float(0)), float4(1,0.7844843,0,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,0.7844843,0,1).rgb, float(0)), float4(1,0.7844843,0,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0) + poiLight.nDotV * float(0), float(0));
				decalColor.a *= applyTilingClipping(float(1), uv) * decalMask[float(0)];
				if (float(0) > 0)
				{
					decalColor.a = customBlend(decalColor.a, poiMods.globalMask[float(0)-1], float(2));
				}
				...
				decalColor = tex2D(_DecalTexture1, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(0.3631418,0.3514183,1,1).rgb, float(0)), float4(0.3631418,0.3514183,1,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) + sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(0.3631418,0.3514183,1,1).rgb, float(0)), float4(0.3631418,0.3514183,1,1).a);
				#endif
				decalColor.rgb = decalHueShift(float(0), decalColor.rgb, float(0) + poiLight.nDotV * float(0), float(0));
				...
				#endif
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, decalColor.rgb, float(0)), decalColor.a * saturate(float(1) + audioLinkDecalAlpha1));
				if (float(0))
				{
					alphaOverride += 1;
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if(float(0.83) != 1)
				{
					subsurface = pow(subsurface, float(0.83));
				}
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_MATCAP) || !defined(OPTIMIZER_ENABLED)
				matcap = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap, _MainTex, TRANSFORM_TEX(matcapUV, _Matcap)) * float4(poiThemeColor(poiMods, float4(0.5754442,0.5754442,0.5754442,1).rgb, float(0)), float4(0.5754442,0.5754442,0.5754442,1).a);
				#else
				matcap = float4(poiThemeColor(poiMods, float4(0.5754442,0.5754442,0.5754442,1).rgb, float(0)), float4(0.5754442,0.5754442,0.5754442,1).a);
				#endif
				matcap.rgb *= float(1);
				#if defined(PROP_MATCAPMASK) || !defined(OPTIMIZER_ENABLED)
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.849));
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.6837681,0.000303527,1).rgb, float(0)), float4(1,0.6837681,0.000303527,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.6837681,0.000303527,1).rgb, float(0)), float4(1,0.6837681,0.000303527,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.35), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0.6837681,0.000303527,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
						...
						if (float(0)) rampedLightMap = 1 - rampedLightMap;
						rim = lerp(rim, rim * rampedLightMap, float(1));
						break;
						case 1:
						float nDotLNormalized = poiLight.nDotLNormalized;
						if (float(0)) nDotLNormalized = 1 - nDotLNormalized;
						rim = lerp(rim, rim * smoothstep(float4(0,0,0,1).x, float4(0,0,0,1).y, nDotLNormalized), float(1));
						break;
					}
				}
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.6837681,0.000303527,1).rgb, float(0)));
			...
			{
				#if defined(PROP_SSSTHICKNESSMAP) || !defined(OPTIMIZER_ENABLED)
				float SSS = 1 - POI2D_SAMPLER_PAN(_SSSThicknessMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				#else
				float SSS = 1;
				#endif
				float3 vLTLight = poiLight.direction + poiMesh.normals[0] * float(0.7);
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), float(4)) * float(0.3);
				#ifdef UNITY_PASS_FORWARDBASE
				float3 fLT = (flTDot) * saturate(SSS + - 1 * float(0));
				#else
				float3 fLT = poiLight.attenuation * (flTDot) * saturate(SSS + - 1 * float(0));
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,1,0,1) * poiLight.attenuation;
			}
			#endif
