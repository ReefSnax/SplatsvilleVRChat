Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/262cadc6df02a8a408c8dbab4758c884"
{
	Properties
	{
 ...
 #define PROP_MATCAPMASK 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, float(1), float(0), float(0), float(0), float(0), matcap, matcapMask, float(0), float(0), float(0), float(2));
				#endif
				//ifex float(1)==0 && float(0)==0 && float(0)==0 && float(0)==0
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(0.75));
