Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/7c14e76f3be7df54cbbc4db746c36e27"
{
	Properties
	{
 ...
 #define PROP_SSSTHICKNESSMAP 
			...
			{
				#if defined(PROP_SSSTHICKNESSMAP) || !defined(OPTIMIZER_ENABLED)
				float SSS = 1 - POI2D_SAMPLER_PAN(_SSSThicknessMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,2,0,0)), float4(0,0,0,0)).r;
				#else
				float SSS = 1;
				#endif
				float3 vLTLight = poiLight.direction + poiMesh.normals[0] * float(0.5);
				float flTDot = pow(saturate(dot(poiCam.viewDir, -vLTLight)), float(5)) * float(0.395);
				#ifdef UNITY_PASS_FORWARDBASE
				float3 fLT = (flTDot) * saturate(SSS + - 1 * float(0));
				#else
				float3 fLT = poiLight.attenuation * (flTDot) * saturate(SSS + - 1 * float(0));
				#endif
				poiLight.finalLightAdd += fLT * poiLight.directColor * float4(1,0.5633239,0.4858491,1) * poiLight.attenuation;
			}
			#endif
			...
			{
				float smoothness = float(0.2);
				float smoothness2 = float(1);
				float metallic = float(0);
				float specularMask = 1;
				float reflectionMask = 1;
				#if defined(PROP_MOCHIEMETALLICMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_MochieMetallicMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,2,0,0)), float4(0,0,0,0).xy);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(1,2,0,0)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,2,0,0)), float4(0,0,0,0)), float(1));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.9528302,0.2269713,0.112362,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.9528302,0.2269713,0.112362,1).a;
				#if defined(PROP_CLIPPINGMASK) || !defined(OPTIMIZER_ENABLED)
				float alphaMask = POI2D_SAMPLER_PAN(_ClippingMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,2,0,0)), float4(0,0,0,0)).r;
				if (float(0))
				{
