Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/5b6e8736869deb24ba4081e7ce2463f4"
{
	Properties
	{
		...
		[sRGBWarning]_MatcapMask ("Mask--{reference_properties:[_MatcapMaskPan, _MatcapMaskUV, _MatcapMaskInvert]}", 2D) = "white" { }
		...
		[sRGBWarning]_Matcap2Mask ("Mask--{reference_properties:[_Matcap2MaskPan, _Matcap2MaskUV, _Matcap2MaskInvert]}", 2D) = "white" { }
 ...
 #define PROP_MATCAP2 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, float(0), float(0), float(0), float(1), float(0), matcap, matcapMask, float(0), float(0), float(0), float(2));
				#endif
				//ifex float(1)==0
				#ifdef COLOR_GRADING_HDR_3D
				...
				if (float(0))
				{
					matcap2Mask = lerp(0, matcap2Mask * TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor), float(1));
				}
				#endif
				...
				blendMatcap(poiLight, poiFragData, poiMods, float(0), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(0), float(0), float(0), float(2));
				#endif
				//ifex float(1)==0 && float(1)==0 && float(0)==0 && float(0)==0
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,1,1,0.7019608).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(1,1,1,0.7019608).a;
