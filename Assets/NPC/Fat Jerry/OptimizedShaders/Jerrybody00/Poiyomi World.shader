Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/eeda77f7f1c181d44aec4c64c92e1dc6"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void applyMatcap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, inout PoiLight poiLight, in PoiMods poiMods)
			{
				...
				blendMatcap(poiLight, poiFragData, poiMods, float(0), float(0), float(0), float(0), float(0), matcap2, matcap2Mask, float(0.08), float(0), float(0), float(2));
				#endif
				//ifex float(1)==0 && float(1)==0 && float(0)==0 && float(0)==0
			}
			#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(1,1,1,0.8666667).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(1,1,1,0.8666667).a;
