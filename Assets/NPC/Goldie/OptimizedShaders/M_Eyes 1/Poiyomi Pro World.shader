Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/eebbc26da9e17f947beb8c7a27e0dcab"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(1,2,0,0.02)) + _Time.x * float4(0,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,2,0,0.02)), float4(0,0,0,0)), float(1));
