Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/55e360d4efedc2648a5893100bc2acab"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(0.6562665,0.6562665,0.6562665,1) * lerp(indirectColor, 1, float(1))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(1));
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(0.6562665,0.6562665,0.6562665,1), float4(0.6562665,0.6562665,0.6562665,1), float(1)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				float3 indirectColor = float4(0.6562665,0.6562665,0.6562665,1).rgb;
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(1));
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.06083423,0.0515377,0.0515377,0.09411765).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.06083423,0.0515377,0.0515377,0.09411765).a;
