Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/dc1c54590dc8ae047bd9f3449bb66c88"
{
	Properties
	{
 ...
 #define PROP_IRIDESCENCEMASK 
			...
			{
				float3 normal = poiMesh.normals[float(0)];
				if (float(0))
				{
				...
				if (float(0) > 0)
				{
					iridescenceMask = customBlend(iridescenceMask, poiMods.globalMask[float(0)-1], float(2));
				}
				poiFragData.baseColor.rgb = lerp(poiFragData.baseColor.rgb, saturate(iridescenceColor.rgb * float(1)), iridescenceColor.a * float(0.048) * iridescenceMask);
				poiFragData.baseColor.rgb += saturate(iridescenceColor.rgb * float(1) * iridescenceColor.a * float(0.125) * iridescenceMask);
				poiFragData.baseColor.rgb *= saturate(lerp(1, iridescenceColor.rgb * float(1), iridescenceColor.a * float(0.027) * iridescenceMask));
				float emissionStrength = float(2.1);
				#ifdef POI_AUDIOLINK
