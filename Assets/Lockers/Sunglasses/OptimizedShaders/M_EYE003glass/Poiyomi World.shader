Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/9d66dd0b65b5f794782f57630bb08b74"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods, in PoiLight poiLight)
			{
				...
				decalColor = tex2D(_DecalTexture, uv + float4(0,0,0,0) * _Time.x, ddxuv, ddyuv) * float4(poiThemeColor(poiMods, float4(1,0,0.430935,1).rgb, float(0)), float4(1,0,0.430935,1).a);
				#else
				uv = decalUV(float(0), float4(1,1,0,0), float4(0.5,0.5,0,0), float(0) + decalRotation, float(0), decalScale, float4(0,0,0,0) +sideMod, float(0), poiMesh, poiCam);
				decalColor = float4(poiThemeColor(poiMods, float4(1,0,0.430935,1).rgb, float(0)), float4(1,0,0.430935,1).a);
				#endif
