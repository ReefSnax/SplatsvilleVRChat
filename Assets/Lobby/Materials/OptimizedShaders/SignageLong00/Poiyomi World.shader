Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/114e30b58ad9bd442b60102f72b10c42"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(-3,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#endif
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(-3,0,0,0)), float(1) * poiMods.detailMask.g);
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(-3,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.5209957,1,0.004024717,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.5209957,1,0.004024717,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.5209957,1,0.004024717,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(1,1,0,0)) + _Time.x * float4(-3,0,0,0));
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(-3,0,0,0)), float(1));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.2788943,1,0,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.2788943,1,0,1).a;
