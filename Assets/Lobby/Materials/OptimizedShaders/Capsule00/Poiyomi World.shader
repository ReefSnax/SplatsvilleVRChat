Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/5b7c296438de0b049a6fef2f25520367"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1009256,0.5652086,0.7924528,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1009256,0.5652086,0.7924528,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.1009256,0.5652086,0.7924528,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			{
				float enviroRimAlpha = saturate(1 - smoothstep(min(float(0.061), float(1)), float(1), poiCam.vDotN));
				...
				#endif
				float3 envRimCol = lerp(0, max(0, (enviroRimColor - float(0)) * poiFragData.baseColor), enviroRimAlpha).rgb * enviroMask * float(0.211);
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0,0.6118413,1,0.3607843).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0,0.6118413,1,0.3607843).a;
