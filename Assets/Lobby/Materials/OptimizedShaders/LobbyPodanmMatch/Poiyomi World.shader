Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/ca6bc3b23349d5848b361c6143ba0708"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMASK 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)) + (dot(pos, float4(0,0,0,0).xyz) * float(20)) + _Time.x * float(10)).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(0,0,0,0).xyz, float(10), float(20), float(10), float(0), pos);
					}
				}
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.2801709,1,0,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.2801709,1,0,1).a;
