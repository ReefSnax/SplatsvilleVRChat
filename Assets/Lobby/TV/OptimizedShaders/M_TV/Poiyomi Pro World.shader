Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/dffd1c5890a454243af973482f77e5b3"
{
	Properties
	{
 ...
 #define PROP_FLIPBOOKTEXARRAY 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(200,200,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
				half3 detailTexture = 0.21763764082 * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#endif
				poiFragData.baseColor.rgb *= LerpWhiteTo(detailTexture * float(1) * unity_ColorSpaceDouble.rgb, poiMods.detailMask.r * float(2));
			}
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(100,100,0,0)), float4(0,0,0,0)).r;
				#else
				float flipBookMask = 1;
				#endif
				if (float(0) > 0)
				{
					flipBookMask = customBlend(flipBookMask, poiMods.globalMask[float(0)-1], float(2));
				}
				...
				if (!float(0))
				{
					if (float(30) != 0)
					{
						currentFrame = ((_Time.y / (1 / float(30))) + float(0)) % totalFrames;
						if (float(0))
						{
				...
				{
					currentFrame = fmod(float(0), totalFrames);
				}
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * float(1) * flipBookMask * flipbookAlpha);
				poiFragData.baseColor = poiFragData.baseColor + flipBookPixel.rgb * float(0) * flipBookMask * flipbookAlpha;
				poiFragData.baseColor = poiFragData.baseColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(1,1,1,1).a * flipBookMask * float(0) * flipbookAlpha);
				float flipbookEmissionStrength = float(1);
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
			...
			{
				float clearCoatMask = float(0.5);
				float smoothness = float(0.572);
				float reflectionMask = 1;
				float specularMask = 1;
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
				float4 PRBMaps = POI2D_SAMPLER_PAN(_ClearCoatMaps, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0));
				clearCoatMask *= PRBMaps.r;
				smoothness *= PRBMaps.g;
				reflectionMask *= PRBMaps.b;
				specularMask *= PRBMaps.a;
				#endif
				specularMask *= float(0.304);
				reflectionMask *= float(0.567);
				if (float(0))
				{
				...
				if (float(0))
				{
					clearCoatMask = lerp(0, clearCoatMask * TPSBufferedDepth(poiMesh.localPos, poiMesh.vertexColor), float(1));
				}
				#endif
				if (float(0))
				{
				...
				float3 reflCol = GetReflections(poiCam, poiLight, poiMesh, roughness, float(0), float(0), _ClearCoatFallback, _ClearCoatFallback_HDR, poiCam.vertexReflectionDir);
				reflections = surfaceReduction * reflCol * FresnelLerp(specCol, grazingTerm, poiLight.vertexNDotV);
				reflections *= poiThemeColor(poiMods, float4(1,1,1,1), float(0)) * reflectionMask;
				#ifdef POI_PASS_ADD
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(5,5,0,0)), float4(0,0,0,0)), float(1));
