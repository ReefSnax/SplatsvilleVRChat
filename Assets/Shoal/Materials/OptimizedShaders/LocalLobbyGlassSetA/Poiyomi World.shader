Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/a55bfe7ee9fe9ae4da2883f8a459dd92"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(1), float(1));
				}
				else
				{
			...
			{
				float clearCoatMask = float(0.371);
				float smoothness = float(1);
				float reflectionMask = 1;
				float specularMask = 1;
				#if defined(PROP_CLEARCOATMAPS) || !defined(OPTIMIZER_ENABLED)
