Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/c0b8767363d0aec4a93b66702c9021ed"
{
	Properties
	{
 ...
 #define PROP_DETAILNORMALMAP 
			...
			void ApplyDetailNormal(inout PoiMods poiMods, inout PoiMesh poiMesh)
			{
				...
				#endif
				#if defined(PROP_DETAILNORMALMAP) || !defined(OPTIMIZER_ENABLED)
				half3 detailNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_DetailNormalMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(100,100,0,0)), float4(2,2,0,0)), float(1) * poiMods.detailMask.g);
			...
			{
				float smoothness = float(0.927);
				float smoothness2 = float(1);
				float metallic = float(0.341);
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(100,100,0,0)), float4(5,5,0,0)), float(1));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.006547666,0.03011692,0.1237992,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.006547666,0.03011692,0.1237992,1).a;
