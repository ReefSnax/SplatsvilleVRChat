Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/98040e55cddcd274f85c620ff83f1d17"
{
	Properties
	{
 ...
 #define POI_VERTEXLIGHT_ON 
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(1.04));
					poiLight.indirectColor = min(poiLight.indirectColor, float(1.04));
				}
				if (float(0))
				{
				...
				poiLight.attenuation = attenuation;
				#endif
				poiLight.directColor = float(1) ? min(float(1), _LightColor0.rgb) : _LightColor0.rgb;
