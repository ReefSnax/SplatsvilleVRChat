Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/5808df04bfca7ce4aabaa4ab0bf27bc6"
{
	Properties
	{
 ...
 #define PROP_MOCHIEMETALLICMAPS 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.057));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.057), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.057));
				}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.057));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.057));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.057))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.057))));
				}
