Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi World/e04c070c4a2a74440bdfd1f6f42d89b0"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			{
				#if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)
				half3 detailTexture = POI2D_SAMPLER_PAN(_DetailTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(1,1,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
				#else
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(1,1,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, float(0));
				}
				else
				{
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.6152565,0.3174468,1,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.6152565,0.3174468,1,1).a;
