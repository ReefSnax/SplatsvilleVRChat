Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro World/d45c42b616a44b246a7c6b36045ea8d3"
{
	Properties
	{
 ...
 #define PROP_EMISSIONMAP 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.finalLighting = lerp(poiLight.directColor, min(lerp(poiLight.indirectColor * float4(1,1,1,1), float4(1,1,1,1), float(1)) * poiLight.occlusion + (sss * poiLight.directColor), poiLight.directColor), float(1));
				#endif
				#ifdef _LIGHTINGMODE_SDF
			...
			{
				float2 st = frac(poiMesh.uv[float(0)] + float4(0,0,0,0).xy * _Time.x) * float(300);
				...
				switch(float(0))
				{
					...
					finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(specAngle, float(300)), 0);
				...
				if (float(0) > 0)
				{
					glitterMask = customBlend(glitterMask, poiMods.globalMask[float(0)-1], float(2));
				}
				if (float(0))
				{
				...
				if (float(0) == 1)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, finalGlitter * glitterColor * float(0.4), finalGlitter * glitterTexture.a * glitterMask);
					poiFragData.emission += finalGlitter * glitterColor * max(0, (float(0.4) - 1) * glitterTexture.a) * glitterMask;
				}
				else
				{
					poiFragData.emission += finalGlitter * glitterColor * float(0.4) * glitterTexture.a * glitterMask;
				}
			}
			#endif
			...
			void applyDepthFX(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				#if defined(PROP_DEPTHMASK) || !defined(OPTIMIZER_ENABLED)
				float depthMask = POI2D_SAMPLER_PAN(_DepthMask, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).r;
				#else
				float depthMask = 1;
				#endif
				if (float(0) > 0)
				{
					depthMask = customBlend(depthMask, poiMods.globalMask[float(0)-1], float(2));
				}
				if (float(0))
				{
					float colorBlendAlpha = lerp(float(0), float(1), remapClamped(float(0), float(1), diff));
					#if defined(PROP_DEPTHTEXTURE) || !defined(OPTIMIZER_ENABLED)
					float2 depthTextureUV = float2(0, 0);
					if (float(0) == 8)
					{
						depthTextureUV = lerp(0, 1, remapClamped(float(0), float(1), diff));
					}
					else
					{
						depthTextureUV = poiMesh.uv[float(0)];
					}
					float3 depthColor = POI2D_SAMPLER_PAN(_DepthTexture, _MainTex, poiUV(depthTextureUV, float4(1,1,0,0)), float4(0,0,0,0)).rgb * poiThemeColor(poiMods, float4(1,1,1,1), float(0));
					#else
					float3 depthColor = poiThemeColor(poiMods, float4(1,1,1,1), float(0));
					#endif
					switch(float(0))
					{
						case 0:
					...
					poiFragData.emission += depthColor * colorBlendAlpha * float(0) * depthMask;
				}
				if (float(0))
				{
					poiFragData.alpha *= lerp(poiFragData.alpha, saturate(lerp(float(1), float(0), remapClamped(float(0), float(1), diff))), depthMask);
				}
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					poiLight.vColor[index] = lerp(unity_LightColor[index].rgb, dot(unity_LightColor[index].rgb, float3(0.299, 0.587, 0.114)), float(1));
				...
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), float(1));
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), float(1));
				if (float(1))
				{
